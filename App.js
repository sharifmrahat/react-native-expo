...
export default function App() {
  ...
  return (
    <View style={styles.container}>
      <ScrollView>
      <Text style={{fontWeight: 'bold', fontSize: 20, padding: 20, textAlign: 'center'}}>Welcome to React Native</Text>
      {/* <StatusBar style="auto" /> */}

      <View style={{width: 200, height: 200, backgroundColor: 'red', marginHorizontal: 20}}/>
{/* 
      <View style={{width: 100, height: 200, backgroundColor: 'blue'}}/> */}
      <Text style={{fontSize: 20,  margin: 20}}>Hello World</Text>

      <Text style={{fontSize: 20, fontWeight: 'bold', margin: 20, textAlign: 'center'}}>Counter: {count}</Text>

      <View style={{flex: 1, flexDirection: 'row-reverse', gap: 10, justifyContent: 'center'}}>
      <Button title='Increase +' onPress={() => setCount(count+1)}/>
      <Button title='Decrease -' onPress={() => setCount(count-1)}/>
      </View>
     
      
      <Image source={require('./assets/adaptive-icon.png')} style={{width: 200, height: 200, borderWidth: 2, borderColor: 'gray', borderRadius: 20, margin: 20}}/>

      <Image source={{uri: 'https://picsum.photos/seed/picsum/200/300'}} style={{width: 300, height: 200, margin: 20}}/>
